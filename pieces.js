...
for (let i = pieces.length - 1; i >= 0; i--) {
  if (pieces[i].prix.description > 35) {
...
for (let i = 0; i < noms.length; i++) {
  const abordablesHeader = description[i];
  const prix = pieces.map((piece) => piece.prix);
  const nomElement = document.createElement("li");
  nomElement.innerText =
    noms[i] + "- " + prix[i] + "€" + ": " + abordablesHeader;
  abordablesElements.appendChild(nomElement);
}
// Ajout de l'en-tête puis de la liste au bloc résultats filtres
document.querySelector(".abordables").appendChild(abordablesElements);
document.querySelector(".abordables").appendChild(abordablesHeader);
